...
#definition("Exchange")[For types $Γ⊢A$, $Γ⊢B$, we have the following isomorphic contexts:
$ Γ,A,B[π_A] ≃ Γ,B,A[π_B] $
We denote the substitution as:
$ Γ,A,B[π_A] ⊢ ex(A,B): (Γ,B,A[π_B]) $
We further require the following judgmental equalities:
$ ex(A,B); $
] <def_exchange>
